...
class ZipHandler:
    ...
    def create_sanitized_zip(self, source_dir: Path, output_path: Path, 
                           original_zip_path: Optional[Path] = None) -> Path:
    ...
    def process_zip_file(self, zip_path: Path, sanitizer, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ZIP processing complete: {results['sanitized_files']}/{results['extracted_files']} files sanitized")
    ...
    def process_zip_to_folder(self, zip_path: Path, sanitizer, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ZIP extraction complete: {results['sanitized_files']}/{results['processed_files']} files sanitized to {output_dir}")
    ...
    def process_zip_in_memory(self, zip_path: Path, sanitizer, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"In-memory ZIP processing complete: {results['sanitized_files']}/{results['processed_files']} files sanitized")
    ...
    def _is_config_file(self, filename: str) -> bool:
        ...
        config_extensions = {
            '.txt', '.conf', '.config', '.cfg', '.Config',
            '.ios', '.nx', '.eos', '.xml'
        }
        
        config_patterns = [
            'running', 'startup', 'config', 'conf',
            'show run', 'show config', 'cfg'
        ]
