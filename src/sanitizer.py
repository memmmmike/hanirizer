...
class NetworkSanitizer:
    ...
    def sanitize_zip_file(self, zip_path: str, output_dir: Optional[str] = None, in_memory: bool = True, output_format: str = 'folder') -> Dict[str, Any]:
    ...
    def sanitize_archive_file(self, archive_path: str, output_dir: Optional[str] = None, 
                             archive_format: str = 'folder', password: Optional[str] = None,
                             output_password: Optional[str] = None) -> Dict[str, Any]:
        ...
        with ArchiveHandler(password=password) as archive_handler:
            try:
                ...
                logger.info(f"Archive processing complete: {results['sanitized_files']}/{results['extracted_files']} files sanitized")
    ...
    def _apply_builtin_patterns(self, content: str) -> Tuple[str, List[str]]:
                ...
                if username_match:
                    ...
                    if username in self.config.service_accounts:
                        ...
                        if username in self.config.personal_accounts:
                            if username not in self._personal_user_mapping:
                                self._personal_user_mapping[username] = f"netadmin{len(self._personal_user_mapping) + 1}"
        ...
        routing_patterns = [
            ...
            (r'authentication mode md5.*key-string \d+ (\S+)', r'authentication mode md5 key-string 7 REDACTED_AUTH_KEY')
        ]
